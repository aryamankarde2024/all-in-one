...
app.post('/register', (req, res) => {
    ...
    connection.query('insert into login (fname, lname, email, username) values (?, ?, ?, ?)', Object.values(values), (err, results) => {
        if(err){
                ...
                if(err){
                    ...
                    if(Date.now() <= results[0].link_valid){
                        ...
                        connection.query(
                            ...
                            [password, salt, Date.now() - 1, urlObj.get('username'), urlObj.get('code')],
                            (err, results) => {
                                if(err){
                                    console.log(err);
                                    res.sendStatus(404);
